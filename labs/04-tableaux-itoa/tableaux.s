// Labo 04 — somme, max et affichage décimal.
...
_start:
    adrp x19, tab@PAGE
    add x19, x19, tab@PAGEOFF   // x19 = adresse du tableau
    mov x20, #5                 // nombre d'éléments
    mov x22, #0                 // somme
    mov x21, #0                 // max

    // TODO : boucle : pour chaque élément (ldr w9, [x19], #4) :
    //   x22 += w9 ; si w9 > x21 alors x21 = w9

    // affichage du max
    mov x0, x21
    adrp x1, buf@PAGE
    add x1, x1, buf@PAGEOFF
    bl itoa                     // x0 = texte, x1 = longueur
    bl print
    adrp x0, nl@PAGE
